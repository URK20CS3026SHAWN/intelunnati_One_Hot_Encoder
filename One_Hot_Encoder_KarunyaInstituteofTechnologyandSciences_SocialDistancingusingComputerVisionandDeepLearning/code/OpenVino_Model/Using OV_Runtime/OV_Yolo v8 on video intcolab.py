...
def plot_boxes(img: np.ndarray, results: np.ndarray = None, color: tuple[int, int, int] = [0, 255, 0], boxes=None,
...
core = Core()
det_ov_model = core.read_model("models/yolov8n.onnx")
device = "CPU"  # "GPU"
if device != "CPU":
...
video_writer = cv2.VideoWriter(
    'demo_videos/OpenVino_Model/Using OV_Runtime/out_'+vid_name, 
    fourcc, 10.0, size)
...
while (cap.isOpened()):
    ...
    if len(results):
        draw_results(results[0], frame)  # draw the results
